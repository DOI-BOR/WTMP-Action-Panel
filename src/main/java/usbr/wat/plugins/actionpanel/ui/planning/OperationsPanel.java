...
import javax.swing.DefaultListModel;             // Backing model for the operations list, also reused by the summary strip box
import javax.swing.JList;                        // Displays this Set's operations entries
...
/**
 * The "Operations" sub-tab of the Planning tab, mirroring the Forecast Conditions
 * workflow's own Operations tab (e.g. "SEP90_WY2024" in the mockup).
 *
 * <p><b>Extension point:</b> the source and shape of Planning-workflow operations data
 * was not specified beyond mirroring the Forecast Conditions tab's structure. This panel
 * currently renders an empty list with the correct layout, tab wiring, and summary-strip
 * integration already in place; wire {@link #setPlanningSet(PlanningSet)} up to the real
 * operations data source once it is defined.</p>
 */
@SuppressWarnings("serial")
public class OperationsPanel extends AbstractPlanningPanel {

	// Backing model for the operations list, shared with the summary strip's mini box
	private final DefaultListModel<Object> _listModel = new DefaultListModel<>();

	// Displays the current Set's operations entries
	...
	public String getTabName() {
		return "Operations"; // Fixed label used for both the tab and its summary-strip box
	}
